...
<script>import { onDestroy, onMount, tick, createEventDispatcher } from 'svelte';
...
if (typeof window !== 'undefined') {
    uid = window.popover_uid = (window.popover_uid || 0) + 1;
    window.popover_uid++;
}
const ALIGN_OPOSITES = {
    auto: 'auto',
    top: 'bottom',
    middle: 'middle',
    bottom: 'top',
 ...
 */
export let align = 'bottom-center';
/**
 * The alignment relative to the anchor
 */
export let alignAnchor = 'auto';
...
function transitionFn(node, params) {
    if (transition) {
...
let parentEl;
let dropdownEl;
let dropdownPointerRectMaskEl;
let dropdownPointerRectMaskFullEl;
let dropdownPointerRectEl;
let dropdownPointerEl;
let dropdownPointerPathEl;
let dropdownPointerRectPathEl;
let childSlotEl;
let interestRects = new Map();
...
function getFlooredBoundingClientRect(el) {
...
function placeAlignment(to, rect, alignOverflow = offset / 2) {
    let x = 0;
    let y = 0;
    let [alignFirst, alignLast] = to.split('-');
    let apply = (alignTo, overflow = 0) => {
        switch (alignTo) {
            case 'right':
...
function calcPosition(force = false) {
    ...
    if (!anyInterestChanged && !force)
        return;
    let targetAnchor = placeAlignment(alignAnchorReal, anchorRect);
    ...
    function solveCollision(rect, axis = 'x' || 'y') {
    ...
    if (constrainToWindow)
        solveContain(windowRect);
    if (avoidOverlap)
        solveCollision(anchorRectWithOffset, 'y');
    for (let col of collideWith) {
        if (col)
            solveCollision(col.getBoundingClientRect(), 'y');
    }
    if (constrainToWindow)
        solveContain(windowRect);
    if (avoidOverlap)
        solveCollision(anchorRectWithOffset, 'x');
    for (let col of collideWith) {
        if (col)
            solveCollision(col.getBoundingClientRect(), 'x');
    }
    if (constrainToWindow)
        solveContain(windowRect);
    if (forcePosition) {
    ...
    let off = offset * 4;
    dropdownPointerRectPathEl?.setAttribute('d', `
...
function calcPointer(dropRect, anchorRect) {
    ...
    dropdownPointerPathEl?.setAttribute('d', ` M${p1.x} ${p1.y}
...
function isElementInPopover(el) {
    let directDescendant = el === dropdownEl || dropdownEl.contains(el);
    if (directDescendant) {
        return true;
    }
    // Resolve popover chain
    let popoverParent = el.closest('[data-popover-id]');
    while (popoverParent) {
        let popoverAnchor = document.querySelector("[data-popover-anchor-id='" + popoverParent.dataset.popoverId + "']");
...
if (typeof window !== 'undefined') {
    ...
    onMount(() => {
        if (!window.popover_elements) {
            window.popover_elements = new Map();
        }
        window.popover_elements.set(uid, {
            parentEl,
            dropdownEl
        });
        function frameLoop() {
